...
1. **The getting-started walkthrough** — pre-flight design decisions, day-by-day milestones, the build-and-test inner loop, common errors and what they actually mean.
...
3. **The parity checklist** — the surface every temporal type must expose to be on equal footing with its peers (`tgeo`, `tpose`, `trgeo`, `tnpoint`, `tcbuffer`).
...
Before writing any code, answer these. Each one constrains downstream choices and writing them down up front avoids weeks of rework.
...
If any answer is "I don't know yet", stop. The runbook assumes you know these. The wrong default for any of them is several days of rework.
...
### 2.2 Day-by-day path with milestones

This is a rough timeline for an experienced contributor (someone who has read this document and shipped at least one MobilityDB PR). Multiply by 2–3× for a first-timer; halve for a small leaf addition (e.g. a new aggregate, not a new type).

| Day | Runbook sections | Milestone — verify with `psql` |
|---|---|---|
| 1–2 | §3.0 (subtree import, if needed), §3.1 (catalog + build), §3.2 (static base type) | `SELECT 'foo(...)'::foo::text;` round-trips. `SELECT foo_eq('foo(1,2)'::foo, 'foo(1,2)'::foo);` returns true. |
| 3 | §3.3 (set type) | `SELECT 'fooset{...}'::fooset::text;` round-trips. `SELECT cardinality('fooset{...}'::fooset);` returns the right count. |
| 4–5 | §3.4 (bbox), §3.5 (lifted temporal type) | `SELECT '{[foo(1,2)@2024-01-01, foo(3,4)@2024-01-02]}'::tfoo;` parses. `SELECT tfoo_to_stbox(...);` returns a non-NULL bbox. |
| 6–7 | §3.6 (index opclasses) | `EXPLAIN SELECT ... WHERE tfoo && stbox(...);` shows a Bitmap Index Scan on the GiST index. The scan returns the right rows (compare against a sequential scan). |
| 8 | §3.7 (registry cache, if applicable) | `meos/examples/standalone_foo.c` populates the cache via `meos_foo_register()` and a query against it returns the right values without a PG backend. |
| 9 | §3.8 (datagen) | `SELECT random_tfoo_inst(...) FROM generate_series(1, 100);` produces 100 plausible values. `tbl_tfoo` table populates without error. |
| 10–11 | §3.9 (regression tests) | `ctest -R tfoo` runs the full suite; expected outputs check in cleanly. |
| 12 | §3.10 (DocBook chapter) | `xmllint --noout --xinclude --noent doc/mobilitydb-manual.xml` is silent. The Doxygen build resolves all `@csqlfn` cross-links. |
| 13–14 | Parity checklist (§4) + robustness pass (§5) + documentation pass (§6) | Every row in the parity table is checked. Each named hazard is classified and mitigated or documented. The DocBook chapter has a `<sect1 xml:id="foo_type_guidance">` covering durability, when-to-use, design restrictions, and the four-to-six named hazards specific to this type. |
...
1. **Defining the value semantics.** What does it mean for two `foo` values to be equal? What does interpolation between two `foo` instants compute? What is the "zero" value, and is it a valid value or a degeneracy? These have to be settled before any code is written, and getting them wrong is several weeks of rework.
...
| MFJSON input | `tinstant_from_mfjson` / `tinstarr_from_mfjson` / `temporal_from_mfjson` branch on `T_TFOO`; SQL `tfooFromMFJSON(text)` | tpose, trgeo, tcbuffer |
...
| Aggregates | `extent` (returns the spatiotemporal `STBox`), `tcount`, `merge`, `appendInstant`, `appendSequence` | every type |
...
| Per-function MEOS unit suite | `meos/test/foo_test.c` mirroring `geo_test.c` / `temporal_test.c` shape; covers every public symbol | tgeo, trgeo |
...
- Recent end-to-end examples to study commit-by-commit:
  ...
  - **`trgeometry`** — parity sweep (`feat/trgeo-distance` family + the trgeo DocBook chapter).
