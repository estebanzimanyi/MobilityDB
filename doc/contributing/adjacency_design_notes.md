...
two rules it replaces are each indefensible.

> **Audience**: a contributor working at the MEOS C level, or anyone revisiting the box
> rule. This is design rationale, not user documentation.
>
> **Status**: every section states current behaviour. The box rule of §5 is **settled**
> and the code carries it.

Every table below transcribes live output on master `fbdc3db1ea`; §7 carries the
reproduction recipe. The one exception is the `R1` and `R2` columns of §5, which record
what those two rules answer and are read from their definitions there — current master
carries R3, so no build produces them.
...
## 5. The boxes: one rule, and the two it replaces
...
The rest of this section is the argument for R3, because the two rules it replaces are
each defensible on first reading and each provably wrong:
...
### 5.5 How far the rule reaches, and what it moved
...
Adopting R3 moves **33 expected-output lines over 10 test files**:
