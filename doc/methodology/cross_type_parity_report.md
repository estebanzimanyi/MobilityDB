...
| Adapter | `tools/parity_audit/adapters/mobilitydb.sql` over `pg_proc` (extension-owned functions, first-arg receiver type) |
...
- `asMVTGeom`, `spaceBoxes`, `spaceSplit`, `spaceTiles`, `spaceTimeBoxes`, `spaceTimeSplit`, `spaceTimeTiles`: a uniform planar coordinate grid / Web-Mercator MVT tile is undefined on the sphere; geodetic space binning uses the H3 family (geoToH3Cell / th3index)
