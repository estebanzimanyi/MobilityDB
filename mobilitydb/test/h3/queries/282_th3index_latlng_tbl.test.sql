...
-- Round trip via th3index at the original resolution: identity
SELECT COUNT(*) FROM tbl_th3index_known
  WHERE th3index(h3_cell_to_latlng(temp),
...
SELECT startValue(h3_get_resolution(th3index(
...
SELECT COUNT(*) FROM (
  ...
  WHERE th3index(p, 5) IS NOT NULL;

-------------------------------------------------------------------------------
-- th3index(tgeompoint, integer)  — SRID 4326
-------------------------------------------------------------------------------
