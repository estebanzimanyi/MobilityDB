        ...
        bool quiet)
{
  ...
  if (OidIsValid(oid))
  {
    ...
    else if (if_not_exists)
    {
      ...
      /* OK to skip */
      if (collencoding == -1)
        elog(NOTICE, "collation \"%s\" already exists, skipping", collname);
      else
        elog(NOTICE, 
      ...
      if (collencoding == -1)
        elog(ERROR, "collation \"%s\" already exists", collname);
      else
        elog(ERROR, "collation \"%s\" for encoding \"%s\" already exists",
          collname, pg_encoding_to_char(collencoding));
  }
