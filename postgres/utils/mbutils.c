 ...
 */
int
pg_do_encoding_conversion_buf(Oid proc, int src_encoding, int dest_encoding,
  unsigned char *src, int srclen, unsigned char *dest, int destlen,
  bool noError)
{
  /*
...
bytea *
pg_convert_to(bytea *string, char *dest_encoding_name)
{
  char *src_encoding_name = DirectFunctionCall1(namein,
...
Datum
pg_convert_from(bytea *string, char *src_encoding_name)
{
 ...
 */
bytea *
pg_convert(bytea *string, char *src_encoding_name, char *dest_encoding_name)
{
 ...
 */
int
length_in_encoding(bytea *string, char *src_encoding_name)
{
  int src_encoding = pg_char_to_encoding(src_encoding_name);
  if (src_encoding < 0)
  {
  ...
  int len = VARSIZE_ANY_EXHDR(string);
  const char *src_str = VARDATA_ANY(string);
  return pg_verify_mbstr_len(src_encoding, src_str, len, false);
}
 ...
 */
Datum
pg_encoding_max_length_sql(int32 encoding)
{
...
/* same, with any encoding */
int
pg_encoding_wchar2mb_with_len(int encoding, const pg_wchar *from, char *to,
  int len)
{
  return pg_wchar_table[encoding].
    wchar2mb_with_len(from, (unsigned char *) to, len);
...
pgwin32_message_to_UTF16(const char *str, int len, int *utf16len)
{
  ...
  UINT codepage = pg_enc2name_tbl[msgenc].codepage;
