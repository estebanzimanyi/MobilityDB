...
text_eq(text *txt1, text *txt2)
{
  ...
  // {
    ...
    // else
    // {
      // text     *targ1 = DatumGetTextPP(txt1);
      // text     *targ2 = DatumGetTextPP(txt2);
...
pg_text_starts_with(text *txt1, text *txt2)
{
  ...
  else
  {
    ...
    text     *targ2 = DatumGetTextPP(txt2d);
...
pg_text_format(PG_FUNCTION_ARGS)
{
  ...
  for (cp = start_ptr; cp < end_ptr; cp++)
  {
    ...
    {
      meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
        "unrecognized format() type specifier \"%.*s\"", pg_mblen(cp), cp);
    }
...
text_format_parse_digits(const char **ptr, const char *end_ptr, int *value)
{
  ...
  while (*cp >= '0' && *cp <= '9')
  {
    ...
    {
      meos_error(ERROR, MEOS_ERR_VALUE_OUT_OF_RANGE, "number is out of range");
      return false;
    }
    ADVANCE_PARSE_POINTER(cp, end_ptr);
    found = true;
