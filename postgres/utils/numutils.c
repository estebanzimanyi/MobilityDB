...
// extern bool pg_neg_u16_overflow(uint16 a, int16 *result);
// extern bool pg_neg_u32_overflow(uint32 a, int32 *result);
// extern bool pg_neg_u64_overflow(uint64 a, int64 *result);
 ...
 */
int16
pg_strtoint16_safe(const char *s, void *escontext)
{
  const char *ptr = s;
 ...
 */
int32
pg_strtoint32_safe(const char *s, void *escontext)
{
  const char *ptr = s;
  const char *firstdigit;
  uint32    tmp = 0;
 ...
 */
int64
pg_strtoint64_safe(const char *s, void *escontext)
{
  const char *ptr = s;
  const char *firstdigit;
  uint64    tmp = 0;
...
uint32in_subr(const char *s, char **endloc,
        const char *typname, void *escontext)
{
  uint32    result;
  unsigned long cvt;
  char     *endptr;
