#!/usr/bin/env python3
"""
close_c6_gaps.py — Insert missing <indexterm> entries into DocBook XML chapters
to close "C6" gaps reported by check_sql_doc_alignment.py.

Each gap is a public SQL function name absent from the DocBook index but
documented via operator symbols. We insert function-name <indexterm> entries
before the closing tag of the relevant section.
"""
import re
import os
...
def find_enclosing_sect_close(content: str, section_id: str):
    """
    Find the position just BEFORE </sect1> or </sect2> that closes the
    outermost sect containing xml:id="section_id".

    Returns (insert_pos, tag_name) or (None, None).
    """
    ...
    while pos < id_pos:
        ...
        if m_open is None and m_close is None:
            break
        elif m_open is not None and (m_close is None or m_open.start() < m_close.start()):
...
def process_file(filepath: str, groups: dict):
    """
    For each (section_id → [fn_names]) group in the file, insert indexterm
    entries before the section's enclosing closing tag.

    Returns total count of entries inserted.
    """
    ...
    for section_id, fn_names in groups.items():
        ...
        insert_pos, tag_name = find_enclosing_sect_close(content, section_id)
...
def main():
    ...
    c6_path = "/tmp/c6_clean.txt"
    if os.path.exists(c6_path):
        with open(c6_path) as fh:
