...
# above defines NDEBUG and EXCLUDES it -- which is how the pin 14l debug-only
# alphanum_temptype parse error (a bad #if H3/#if QUADBIN merge: trailing `||`
# with no leading `||`, plus a duplicated tail) shipped to every vcpkg
# x64-linux-dbg standalone -DMEOS=ON binding while every Release build stayed
# green. This gate makes debug-only regressions fail HERE, before a pin is cut.
