...
def run_checks(sql_fns, pg_wrappers, def_groups, documented,
    ...
    if 'C3' in checks:
        ...
        if no_group:
            ...
            ok(f"All non-infrastructure C wrappers have @ingroup tags")
...
def main():
    ...
    checks = set(DEFAULT_CHECKS) if args.check == 'all' \
             else {c.strip().upper() for c in args.check.split(',')}
