...
static char *jsonPathToCstring(StringInfo out, JsonPath *jp,
  int estimated_len);
static bool flattenJsonPathParseItem(StringInfo buf, int *result,
  struct Node *escontext, JsonPathParseItem *item, int nestingLevel,
...
/**
 * @ingroup meos_json_base_jsonpath
 * @brief Return a copy of a JSON path
 * @param[in] jp JSON path
 * @note MEOS function
 */
#if MEOS
JsonPath *
jsonpath_copy(const JsonPath *jp)
{
  return pg_jsonpath_copy(jp);
}
#endif /* MEOS */
JsonPath *
pg_jsonpath_copy(const JsonPath *jp)
{
 ...
 */
static JsonPath *
jsonPathFromCstring(char *str, int len, struct Node *escontext)
{
  JsonPathParseResult *jsonpath = parsejsonpath(str, len, escontext);
  if (! jsonpath)
  {
    meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
      "invalid input syntax for type jsonpath: \"%s\"", str);
    return NULL;
  }
 ...
 */
static char *
jsonPathToCstring(StringInfo out, JsonPath *jp, int estimated_len)
{
  StringInfoData buf;
  JsonPathItem v;
  ...
  JsonPathParseItem *item, int nestingLevel, bool insideArraySubscript)
{
  ...
  switch (item->type)
  {
    ...
    case jpiDecimal:
      {
        ...
        if (! item->value.args.left)
          chld = pos;
                ...
                item->value.args.left, nestingLevel + argNestingLevel,
        ...
        *(int32 *) (buf->data + left) = chld - pos;
        ...
        if (! item->value.args.right)
          chld = pos;
                  ...
                  item->value.args.right, nestingLevel + argNestingLevel,
                  insideArraySubscript))
          return false;
    ...
    case jpiLikeRegex:
      {
        int32 offs;
        appendBinaryStringInfo(buf, &item->value.like_regex.flags,
          sizeof(item->value.like_regex.flags));
        offs = reserveSpaceForItemPointer(buf);
        appendBinaryStringInfo(buf, &item->value.like_regex.patternlen,
        ...
        if (! flattenJsonPathParseItem(buf, &chld, escontext,
            item->value.like_regex.expr, nestingLevel, insideArraySubscript))
          return false;
    ...
    case jpiTimestampTz:
      {
        ...
        if (! item->value.arg)
          chld = pos;
                 ...
                 item->value.arg, nestingLevel + argNestingLevel,
                 insideArraySubscript))
          return false;
    ...
    case jpiIndexArray:
      {
        ...
        for (i = 0; i < nelems; i++)
        {
          ...
          if (! flattenJsonPathParseItem(buf, &frompos, escontext,
              item->value.array.elems[i].from, nestingLevel, true))
            return false;
          frompos -= pos;
          ...
          if (item->value.array.elems[i].to)
          {
            if (! flattenJsonPathParseItem(buf, &topos, escontext,
                item->value.array.elems[i].to, nestingLevel, true))
              return false;
            topos -= pos;
          }
          else
      ...
      break;
    case jpiAny:
      appendBinaryStringInfo(buf, &item->value.anybounds.first,
        sizeof(item->value.anybounds.first));
      appendBinaryStringInfo(buf, &item->value.anybounds.last,
    ...
    {
      meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
        "unrecognized jsonpath item type: %d", item->type);
      return false;
    }
  }
  ...
  if (item->next)
  {
    if (! flattenJsonPathParseItem(buf, &chld, escontext, item->next,
      nestingLevel, insideArraySubscript))
      return false;
    chld -= pos;
  ...
  bool printBracketes)
{
  ...
  switch (v->type)
  {
    ...
    case jpiNumeric:
    {
      ...
      char *str = pg_numeric_out(jspGetNumeric(v));
    ...
    case jpiStartsWith:
      ...
      printJsonPathItem(buf, &elem, false, operationPriority(elem.type) <=
    ...
    case jpiLikeRegex:
      ...
      escape_json_with_len(buf, v->content.like_regex.pattern,
    ...
    default:
      {
        meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
          "unrecognized jsonpath item type: %d", v->type);
      }
  }
  ...
  while (! cxt->mutable)
  {
    ...
    switch (jpi->type)
    {
      ...
      case jpiGreaterOrEqual:
         ...
         * is mutable.
         */
        if (leftStatus != jpdsNonDateTime && rightStatus != jpdsNonDateTime &&
           (leftStatus == jpdsUnknownDateTime ||
            rightStatus == jpdsUnknownDateTime || leftStatus != rightStatus))
          cxt->mutable = true;
        break;
      ...
      case jpiDatetime:
        if (jpi->content.arg)
        {
          ...
          char *template = jspGetString(&arg, NULL);
