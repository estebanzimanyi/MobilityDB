...
static regexp_matches_ctx *setup_regexp_matches(text *orig_str, text *pattern,
  pg_re_flags *re_flags, int start_search, Oid collation, bool use_subpatterns,
  bool ignore_degenerate, bool fetching_unmatched);
  ...
  int start_search, int nmatch, regmatch_t *pmatch)
{
  ...
  regexec_result = pg_regexec(re, data, data_len, start_search, NULL, nmatch,
    pmatch, 0);
 ...
 */
bool
RE_compile_and_execute(text *text_re, char *dat, int dat_len, int cflags,
  Oid collation, int nmatch, regmatch_t *pmatch)
{
  regex_t *re;
...
bool
nameregexeq(Name n, text *p)
{
  return RE_compile_and_execute(p, NameStr(*n), strlen(NameStr(*n)),
    REG_ADVANCED, PG_GET_COLLATION(), 0, NULL));
}

bool
nameregexne(Name n, text *p)
{
  return ! RE_compile_and_execute(p, NameStr(*n), strlen(NameStr(*n)),
    REG_ADVANCED, PG_GET_COLLATION(), 0, NULL));
}
...
bool
textregexeq(text *s, text *p)
{
  return RE_compile_and_execute(p, VARDATA_ANY(s), VARSIZE_ANY_EXHDR(s),
    REG_ADVANCED, PG_GET_COLLATION(), 0, NULL));
}

bool
textregexne(text *s, text *p)
{
  return ! RE_compile_and_execute(p, VARDATA_ANY(s), VARSIZE_ANY_EXHDR(s),
    REG_ADVANCED, PG_GET_COLLATION(), 0, NULL);
}
...
textregexsubstr(text *s, text *p)
{
  ...
  if (! RE_execute(re, VARDATA_ANY(s), VARSIZE_ANY_EXHDR(s), 2, pmatch))
    return NULL;    /* definitely no match */
  ...
  return return pg_text_substr(s, so + 1, eo - so);
}
 ...
 */
text *
textregexreplace_noopt(text *s, text *p, text *r)
{
  return replace_text_regexp(s, p, r, REG_ADVANCED, PG_GET_COLLATION(), 0, 1);
}
 ...
 */
text *
textregexreplace(text *s, text *p, text *r, text *opt)
{
  /*
  ...
  return replace_text_regexp(s, p, r, flags.cflags, PG_GET_COLLATION(),
    0, flags.glob ? 0 : 1));
}
 ...
 */
text *
textregexreplace_extended(text *s, text *p, text *r, int start, int n)
{
  int start = 1;
  int n = 1;
  ...
  return replace_text_regexp(s, p, r, re_flags.cflags, PG_GET_COLLATION(),
    start - 1, n));
}
...
similar_escape_internal(text *pat_text, text *esc_text)
{
  ...
  else
  {
    ...
    else if (elen > 1)
    {
      ...
      if (escape_mblen > 1)
      {
        meos_error(ERROR, MEOS_ERRCODE_INVALID_ESCAPE_SEQUENCE,
  ...
  while (plen > 0)
  {
    ...
    if (afterescape)
    {
      if (pchar == '"' && bracket_depth < 1)  /* escape-double-quote? */
      {
        ...
        {
          meos_error(ERROR, MEOS_ERR_INVALID_USE_OF_ESCAPE_CHARACTER),
 ...
 */
text *
similar_to_escape_2(text *pat_text, text *esc_text)
{
  return similar_escape_internal(pat_text, esc_text);
}
 ...
 */
text *
similar_to_escape_1(text *pat_text, text *)
{
  return similar_escape_internal(pat_text, NULL);
}
 ...
 */
int
regexp_count(text *str, text *pattern, int start, text *flags)
{
  pg_re_flags re_flags;
  ...
  matchctx = setup_regexp_matches(str, pattern, &re_flags, start - 1,
    PG_GET_COLLATION(), false, false, false);
 ...
 */
Datum
regexp_instr(text *str, text *pattern, int start, int n, int endoption,
  text *flags, int subexpr)
{
   = 0;
  int pos;
  pg_re_flags re_flags;
  ...
  if (matchctx->match_locs[pos] >= 0)
    return matchctx->match_locs[pos] + 1;
  else
    return  0;    /* position not identifiable */
}
 ...
 */
bool
regexp_like(text *str, text *pattern, text *flags)
{
  /* Determine options */
  pg_re_flags re_flags;
  parse_re_flags(&re_flags, flags);
  ...
  return RE_compile_and_execute(pattern, VARDATA_ANY(str),
    VARSIZE_ANY_EXHDR(str), re_flags.cflags, PG_GET_COLLATION(), 0, NULL));
}
 ...
 */
Datum
regexp_match(text *orig_str, text *pattern, text *flags)
{
  pg_re_flags re_flags;
  regexp_matches_ctx *matchctx;
...
setup_regexp_matches(text *orig_str, text *pattern, pg_re_flags *re_flags,
  int start_search, Oid collation, bool use_subpatterns,
  bool ignore_degenerate, bool fetching_unmatched)
{
...
build_regexp_match_result(regexp_matches_ctx *matchctx)
{
  ...
  for (i = 0; i < matchctx->npatterns; i++)
  {
    ...
    {
      elems[i] = pg_text_substr(matchctx->orig_str, so + 1, eo - so);
      nulls[i] = false;
    }
  }
  ...
  return construct_md_array(elems, nulls, 1, dims, lbs, TEXTOID, -1, false,
    TYPALIGN_INT);
}
...
regexp_split_to_array(text *txt, text *pattern, text *flags, int *count)
{
  ...
  {
    astate = accumArrayResult(astate, build_regexp_split_result(splitctx),
 ...
 */
text *
regexp_substr(text *str, text *pattern, int start, int n, text *flags,
  int subexpr)
{
  int so, eo, pos;
  pg_re_flags re_flags;
  ...
  if (so < 0 || eo < 0)
    return NULL; /* unidentifiable location */
