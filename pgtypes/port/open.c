...
/* MEOS: build the PostgreSQL win32 port in frontend mode for standalone
 * MEOS. MEOS provides meos_error instead of the backend elog/ereport
 * machinery (so the #ifndef FRONTEND ereport blocks compile out), and
 * pgtypes has no postgres_fe.h, so include c.h directly. */
#ifndef FRONTEND
#define FRONTEND 1
#endif
#include "c.h"
