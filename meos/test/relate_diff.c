 ...
 * accepts anything.
 *
 * Which engine answers is the dispatch of #geom_relate, not a mode of this
 * program: the native engine answers a pair holding a circular arc, and
 * answers everything it covers where the library carries no GEOS. Linking
 * this against a `-DGEOS=OFF` library is therefore what puts the native
 * engine under test over a corpus of straight geometries, while a library
 * carrying GEOS tests the arc path and the dispatch. The corpus of expected
 * matrices comes from the GEOS suite through `relate_harvest_geos.py`, so
 * the two answer the same question about the same records.
 */
...
main(int argc, char **argv)
{
  ...
  while (getline(&line, &linesize, stdin) > 0)
  {
    ...
    *sep = '\0';
    ...
    char *answer = geom_relate(gs1, gs2);
    bool covered = (answer != NULL);
    if (covered)
    {
