...
Node *private_splay_find_in_tree(SplayTree tree, void *value, 
                                 void *comparison_func(void *val1, void *val2));
void private_SplayTreeransplant(SplayTree tree, Node *u, Node *v); 
...
void SplayTreeo_root(SplayTree tree, Node *n); 
...
{
        SplayTree tree = malloc(sizeof(struct SplayTreeree)); 
...
void splay_delete_value(SplayTree tree, void *value)
{
        ...
        SplayTreeo_root(tree, z); 

        if (z->left == NULL) {
                private_SplayTreeransplant(tree, z, z->right); 
        } else if (z->right == NULL) {
                private_SplayTreeransplant(tree, z, z->left);
        } else {
                Node *y = private_splay_minimum(z->right); 
                if (y->parent != z) {
                        private_SplayTreeransplant(tree, y, y->right); 
                        y->right = z->right; 
                        y->right->parent = y; 
                }
                private_SplayTreeransplant(tree, z, y); 
                y->left = z->left; 
                y->left->parent = y; 
        }
...
void *SplayTreeree_minimum(SplayTree tree)
{
        ...
        if (n == NULL) {
                ...
                SplayTreeo_root(tree, n); 
                return n->value;
        }
}

void *SplayTreeree_maximum(SplayTree tree)
{
