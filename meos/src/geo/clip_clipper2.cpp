 ...
 * input vertices (after CLIP_SCALE quantisation) or was generated by
 * Clipper2 at a polygon-edge crossing of one of the trajectory segments.
 * In the latter case Clipper2's int64 quantisation can leave the vertex
 * slightly off the source segment's exact line (perpendicular error of a
 * few CLIP_SCALE units), so we cannot require exact collinearity. Instead
 * we score every segment whose parameter range admits @p p (via
 * 0 ≤ (p−A)·(B−A) ≤ |B−A|²) by perpendicular distance squared
 * (cross² / |B−A|²) and pick the smallest. Once the segment is chosen the
 * fractional parameter f = (p−A)·(B−A) / |B−A|² is converted to
 * long double for the final timestamp interpolation (same precision as
 * tsegment_value_at_timestamptz elsewhere in MEOS).
 *
  ...
  const std::vector<TimestampTz> &ts, size_t n)
{
  ...
  /* Pick the segment that minimises perpendicular distance to p, restricted
   * to those whose parameter is in [0, 1]. cross² / lensq is monotone in
   * perpendicular distance, so we minimise cross² * other_lensq vs.
   * best_cross² * lensq to avoid the division. */
  bool   have_best = false;
  size_t best_i = 0;
  __int128 best_cross_sq = 0;
  __int128 best_lensq = 1;
  __int128 best_dot = 0;
  for (size_t i = 0; i + 1 < n; i++)
  {
    ...
    if (dot < 0 || dot > lensq)
      continue;  /* projection falls outside this segment */

    __int128 cross = px * dy - py * dx;
    __int128 cross_sq = cross * cross;
    /* cross_sq / lensq < best_cross_sq / best_lensq ↔
     * cross_sq * best_lensq < best_cross_sq * lensq (all non-negative). */
    if (! have_best ||
        cross_sq * best_lensq < best_cross_sq * lensq)
    {
      have_best = true;
      best_i = i;
      best_cross_sq = cross_sq;
      best_lensq = lensq;
      best_dot = dot;
    }
  }
  ...
  if (! have_best)
    return ts[0];  /* unreachable for well-formed input */

  long double f = static_cast<long double>(best_dot) /
    static_cast<long double>(best_lensq);
  long double dt = static_cast<long double>(ts[best_i + 1]) -
    static_cast<long double>(ts[best_i]);
  return static_cast<TimestampTz>(
    static_cast<long double>(ts[best_i]) + f * dt);
}
